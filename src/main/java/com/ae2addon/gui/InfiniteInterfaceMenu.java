...
public class InfiniteInterfaceMenu extends AbstractContainerMenu {
    ...
    public InfiniteInterfaceMenu(int id, Inventory playerInventory, InfiniteInterfaceBE feeder) {
        ...
        Integer ts = foundTool == null ? null : foundTool.getPlayerInventorySlot();
        this.toolSlot = ts == null ? -1 : ts;
        if (toolHost != null) {
            var toolContainer = toolHost.getInventory().toContainer();
            for (int i = 0; i < 9; i++) {
                int col = i % 3;
                int row = i / 3;
    ...
    public void broadcastChanges() {
        ...
        if (!key.equals(lastStatusKey)) {
            ...
            if (opener instanceof ServerPlayer serverPlayer) {
                PacketDistributor.sendToPlayer(serverPlayer, new FeederStatusPacket(lines));
            }
        }
    ...
    /** Component → JSON（客户端反序列化按语言渲染）。1.21.1：toJson 需要 registries。 */
    private String json(net.minecraft.network.chat.Component c) {
        return net.minecraft.network.chat.Component.Serializer.toJson(c, opener.registryAccess());
    }
}
