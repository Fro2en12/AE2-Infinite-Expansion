...
public class IntegratedCPUMenu extends CraftingCPUMenu {
    ...
    // 客户端构造（NeoForge IMenuTypeExtension 工厂）：从网络包读 locator 定位 host
     ...
     */
    public static boolean openMenu(Player player, MenuHostLocator locator,
        ...
        if (owner != null) {
            ...
            if (!key.equals(lastLanesKey)) {
                ...
                if (getPlayer() instanceof net.minecraft.server.level.ServerPlayer sp) {
                    PacketDistributor.sendToPlayer(sp, new com.ae2addon.network.LaneListPacket(
    ...
    private void syncOrders() {
        ...
        if (!key.equals(lastOrdersKey)) {
            lastOrdersKey = key;
            if (getPlayer() instanceof net.minecraft.server.level.ServerPlayer sp) {
                PacketDistributor.sendToPlayer(sp, new com.ae2addon.network.OrderListPacket(descs));
            }
        }
    }

    /** 订单面板行描述：物品 + 进度 + 状态（JSON 序列化 Component，客户端本地化）。1.21.1：toJson 需要 registries。 */
    private String describeOrder(com.ae2addon.crafting.BatchedCraftingOrder order) {
        String itemName = "?";
        try {
        ...
        return Component.Serializer.toJson(desc, registryAccess());
    }
     ...
     */
    private String describeLane(int index, CraftingCPUCluster lane) {
