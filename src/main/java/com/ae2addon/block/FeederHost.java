...
/**
 * 供料站宿主抽象（2026-09-02 part 面板解耦，随上游 v1.3.0 同步）：
 * GUI/菜单/开关/内存卡共用的操作面。方块版 {@link InfiniteInterfaceBE}
 * 与线缆部件版 part 都实现它——菜单/Screen 只依赖此接口。
 */
public interface FeederHost {
    ...
    Map<AEKey, Long> markerTargetsSnapshot();
    ...
    List<Component> reservoirTooltipLines();
    ...
    default boolean insertUpgradeCards(ItemStack held, int limit) {
        if (held.isEmpty() || !appeng.api.upgrades.Upgrades.isUpgradeCardItem(held.getItem())) {
            return false;
        }
    ...
    /** Jade 等外部显示：蓄水池条目按类型分组，每类最多 3 行（其余隐去）。 */
    static List<Component> buildReservoirLines(Map<AEKey, BigInteger> reservoir) {
        return buildReservoirLines(reservoir, 3);
    }

    /** 通用实现：物品/流体/化学物分组，每类限行数，超出显示「…等 N 种」；文本 translatable（2026-09-03 i18n）。 */
    static List<Component> buildReservoirLines(Map<AEKey, BigInteger> reservoir, int perTypeLimit) {
        List<Component> items = new ArrayList<>();
        List<Component> fluids = new ArrayList<>();
        List<Component> chems = new ArrayList<>();
        ...
        for (var e : reservoir.entrySet()) {
            ...
            try {
                ...
                line = e.getKey().getDisplayName().copy()
                        .append(" §7x" + InfiniteInterfaceBE.fmt(e.getValue()));
            } catch (RuntimeException ignored) {
                continue;
            }
            AEKey key = e.getKey();
    ...
    private static void appendGroup(List<Component> out, String headerKey,
            List<Component> group, int limit) {
        if (group.isEmpty()) {
            return;
        }
        out.add(Component.translatable(headerKey));
        for (int i = 0; i < Math.min(group.size(), limit); i++) {
            out.add(Component.literal("  ").copy().append(group.get(i)));
        }
        if (group.size() > limit) {
            out.add(Component.translatable("ae2addon.jade.more", group.size() - limit));
        }
    }
}
