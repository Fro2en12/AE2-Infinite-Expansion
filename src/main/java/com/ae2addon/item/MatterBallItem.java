...
public class MatterBallItem extends Item {
    ...
    /** 1.21：读/写自定义 NBT 走 DataComponents.CUSTOM_DATA（getOrCreateTag 已移除）。 */
    ...
    /** 创建物质球：记录物品 key + 数量 */
    public static ItemStack makeBall(AEItemKey itemKey, long amount, HolderLookup.Provider registries) {
        ItemStack ball = new ItemStack(ModItems.MATTER_BALL.get());
        CompoundTag tag = getOrCreateTag(ball);
        tag.put(NBT_KEY, itemKey.toTagGeneric(registries));
    ...
    /** 读取球内物品 key（无则 null） */
    @Nullable
    public static AEItemKey getKey(ItemStack ball, HolderLookup.Provider registries) {
        if (ball.isEmpty()) return null;
        CompoundTag tag = getTag(ball);
        if (tag == null || !tag.contains(NBT_KEY)) return null;
        AEKey key = AEKey.fromTagGeneric(registries, tag.getCompound(NBT_KEY));
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        AEItemKey key = getKey(ball, level.registryAccess());
        ...
        if (remaining <= 0) {
            ...
            // 部分取出 → 更新球内数量
            getOrCreateTag(ball).putLong(NBT_AMOUNT, remaining);
            player.sendSystemMessage(Component.translatable(
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        AEItemKey key = registries == null ? null : getKey(stack, registries);
